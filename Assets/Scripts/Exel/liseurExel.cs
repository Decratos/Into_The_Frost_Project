...
[System.Serializable]
public class InfoMateriaux // Liste de tous les mat�riaux
{
    public int ID;
    public string Name;
...
public class liseurExel : MesFonctions // ce script vas chercher toutes les infos dans les excel et les stocker, pour permettre d'aller chercher toute les donn�s
{
    ...
    void Awake() // charge les infos
    {
        ...
        DontDestroyOnLoad(this.gameObject); // Permet de garder les infos entre chaque sc�ne 
    ...
    void readEachCSV() 
    {
        ...
        {
            string[] data = MesPages[i].MaPage.text.Split(new string[] { ",", "\n" }, StringSplitOptions.None);// split les donn�es dans un tableau
    ...
    float MonParse (string value) //fonction pour le tableur
    {
        ...
        //print("la valuer modifi�" + Resultat);
    ...
    // bool pour v�rifier si dans liste des crafts
    
   

    #endregion

    #region methode Accessible � tous
    ...
    public void FindObjectInfo(string name, out InfoGlobalExel InfoGlobal) 
    {
        ...
        InfoGlobalExel lesInfos = new InfoGlobalExel(); // cr�er une nouvelle struct
        ...
        foreach (InfoGeneral info in MesListe.LesItems) // pour chaque objet dans la liste
        {
            if (name == info.Name) // je regarde si le nom est bon
            {
                ...
                lesInfos.Durability = info.Durability; // set la durabilit�
                ...
                string [] EnumType = System.Enum.GetNames(typeof(InfoGlobalExel.Type));// cr�er un tableau de string des types
                for (int i = 0; i < EnumType.Length-1; i++) // pour chaque type dans l'enum
                {
                    ...
                    if (EnumType[i] == correctedType) // si l'enum est �gale au type
                    {
                        lesInfos.TypeGeneral = (InfoGlobalExel.Type)i; // set le type g�n�ral
                        break; // break le for
                    }
                }
                if (correctedType == InfoGlobalExel.Type.Nourriture.ToString()) // si c'est de la bouffe
                {
                    lesInfos.exelNourriturre = new InfoExelNourriturre(); // je cr�er une type bouffe
                ...
                break; // j'ai trouv� ma cible je peux casser
            }
        }
        if (lesInfos.ID == 0) // si l'objet n'est pas dans la base de donner
        {
            print("l'item n'est pas dans la base de donn�e");
            Debug.Break();
    ...
    }//r�cup�rre les donn�es de la bouffe;
    ...
    }//r�cup�rre les donn�es des soins;
    ...
    }//r�cup�rre les donn�es des craft;
    public void FindObjectInfo(string name, out InfoExelArme ArmeInfo) // r�cup�re les donn�es des armes de m�l�e
    {
    ...
    public void FindObjectInfo(int ID, out InfoExelArme ArmeInfo) // r�cup�re les donn�es des armes de m�l�e
    {
        ArmeInfo = new InfoExelArme();
        bool found = false;
        foreach (InfoWeapon arme in MesListe.LesArmes)
    ...
    public void findObjectsByType(PageExel.TypeDePageExel mon_type , out int[]ID)
    {
        ...
        {
            ID = new int[0];
            print("je n'ai pas trouv� l'objet du type" + mon_type.ToString());
            Debug.Break();
        }
    ...
    public void findObjectByChar(char[] Meslettres, out int[] LesID) 
    {
        ...
        {
            print("Je n'ai pas de r�sultats");
            LesID = new int[0];
        }
        // prendre la liste compl�te
